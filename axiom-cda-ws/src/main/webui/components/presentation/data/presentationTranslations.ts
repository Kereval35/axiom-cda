...
export const slideTranslations: Record<Language, PresentationTranslation> = {
  en: {
    ...
    slide3: {
      ...
      fhirProfiles: '→ FHIR FSH from selected CDA IR + built-in Observation presets or custom StructureMap JSON',
    ...
    slide18: {
      ...
      output: {
        ...
        convertFhir: 'Generate FHIR FSH from the WS UI, then compile with SUSHI as a separate step',
      },
    },
    slide20: {
    ...
    slide21: {
      ...
      roadmap: {
        ...
        ci: 'Extend SUSHI and FHIR conversion validation coverage across built-in presets and upload overrides',
    ...
    slide22: {
      ...
      validateWithSushi: 'Generate FHIR FSH from a selected profile, inspect mapping traces, then compile with SUSHI',
    },
    slide23: {
      ...
      why: {
        ...
        description: 'A CDA to FHIR rule-mapping flow: BBR is normalized into a CDA IR, mapping rules are resolved from a built-in preset or uploaded StructureMap JSON, a FHIR-oriented projection is computed, and FHIR FSH is emitted as the artifact.',
      },
      flow: {
        title: 'Pipeline',
        bbrToIr: 'BBR export → CDA IR with normalized paths, datatypes and constraints',
        mappingRules: 'Built-in Observation presets or uploaded StructureMap rules decide how CDA branches project into FHIR semantics',
        fhirProjection: 'FHIR-oriented intermediate projection keeps parent, cardinalities, bindings and fixed values explicit',
        fshOutput: 'Generated FHIR FSH stays traceable to the selected CDA IR template and source FSH profile',
      },
      poc: 'Built-in presets are currently shipped for Observation roots only; non-Observation roots require an uploaded StructureMap JSON override.',
    },
    slide24: {
      ...
      generic: {
        ...
        description: 'Default built-in preset shown only for Observation roots. It targets base HL7 R4 Observation when no IG-specific parent is required.',
      },
      ehdsi: {
        title: 'eHDSI Laboratory Observation',
        description: 'Second shipped Observation preset for projects that need the specialized laboratory parent profile.',
      },
      custom: {
        title: 'Custom StructureMap override',
        description: 'A project can upload StructureMap JSON. This path is required when no built-in preset exists for the selected root.',
      },
      compileNote: 'SUSHI compilation remains a separate step. Base Observation needs no extra IG package; external parents require package id and version.',
    },
    slide25: {
      title: 'Current FHIR conversion scope',
      subtitle: 'Built-in packs, overrides, and community direction',
      v1: {
        title: 'What is shipped today',
        observation: 'Observation is the first root with built-in mapping presets.',
        safety: 'Other roots can enter the conversion page, but need an uploaded StructureMap JSON and use the SPI-selected generator path.',
        business: 'The shipped presets demonstrate both base HL7 output and eHDSI laboratory specialization.',
      },
      community: {
        title: 'Where this goes next',
        reusablePacks: 'Turn mapping rules into reusable packs that can be contributed and reviewed.',
        moreRoots: 'Add reviewed built-in packs for additional CDA roots beyond Observation.',
  ...
  fr: {
    ...
    slide3: {
      ...
      invariants: '→ Invariants (FSH)',
      terminologies: '→ ValueSets / CodeSystems',
      fhirProfiles: '→ FHIR FSH depuis un IR CDA sélectionné + presets Observation ou StructureMap JSON custom',
    ...
    slide18: {
      ...
      output: {
        ...
        convertFhir: 'Générer du FHIR FSH depuis la WS UI, puis compiler avec SUSHI dans une étape séparée',
      },
    },
    slide20: {
    ...
    slide21: {
      ...
      roadmap: {
        ...
        ci: 'Étendre la couverture de validation SUSHI et conversion FHIR sur presets intégrés et surcharges uploadées',
    ...
    slide22: {
      ...
      validateWithSushi: 'Génération FHIR FSH depuis un profil sélectionné, inspection des traces de mapping, puis compilation SUSHI',
    },
    slide23: {
      ...
      why: {
        ...
        description: "Un flux de mapping CDA vers FHIR : le BBR est normalisé en IR CDA, les règles viennent d'un preset intégré ou d'une StructureMap JSON uploadée, une projection orientée FHIR est calculée, puis le FHIR FSH est émis comme artefact.",
      },
      flow: {
        title: 'Pipeline',
        bbrToIr: 'Export BBR → IR CDA avec normalisation des chemins, datatypes et contraintes',
        mappingRules: 'Des presets Observation intégrés ou des règles StructureMap uploadées décident comment les branches CDA se projettent en FHIR',
        ...
        fshOutput: "Le FHIR FSH généré reste traçable jusqu'au template IR CDA et au profil FSH source sélectionnés",
      },
      poc: "Les presets intégrés fournis aujourd'hui concernent uniquement les racines Observation ; les autres racines nécessitent une StructureMap JSON uploadée.",
    },
    slide24: {
      ...
      generic: {
        ...
        description: "Preset intégré par défaut, affiché uniquement pour les racines Observation. Il cible l'Observation HL7 R4 de base lorsqu'aucun parent IG spécifique n'est requis.",
      },
      ehdsi: {
        title: 'Observation laboratoire eHDSI',
        description: "Second preset Observation fourni pour les projets qui ont besoin du parent laboratoire spécialisé.",
      },
      custom: {
        title: 'Surcharge StructureMap personnalisée',
        description: "Un projet peut téléverser sa StructureMap JSON. Ce chemin est obligatoire quand aucun preset intégré n'existe pour la racine sélectionnée.",
      },
      compileNote: "La compilation SUSHI reste séparée. Observation de base ne nécessite pas d'IG externe ; les parents externes exigent un package id et une version.",
    },
    slide25: {
      title: 'Périmètre actuel de conversion FHIR',
      subtitle: 'Packs intégrés, surcharges et ouverture communautaire',
      v1: {
        title: "Ce qui est fourni aujourd'hui",
        observation: 'Observation est la première racine avec des presets de mapping intégrés.',
        safety: "Les autres racines peuvent entrer dans la page de conversion, mais nécessitent une StructureMap JSON uploadée et utilisent le générateur sélectionné via SPI.",
        business: 'Les presets fournis démontrent à la fois une sortie HL7 de base et une spécialisation laboratoire eHDSI.',
      },
      community: {
        ...
        moreRoots: "Ajouter des packs intégrés revus pour d'autres racines CDA au-delà d'Observation.",
